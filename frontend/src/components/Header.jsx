import { useEffect, useState } from "react"
import { Link, useLocation, useNavigate } from "react-router"
import useSearchStore from "../store/searchStore"
import useAuthStore from "../store/authStore"
import useCartStore from "../store/cartStore"

const Header = () => {
    const navigate = useNavigate()
    const location = useLocation()
    const isProductsRoute = location.pathname === "/products"
    const { searchTerm, setSearchTerm } = useSearchStore()
    const [inputValue, setInputValue] = useState(searchTerm)
    const { user, logout } = useAuthStore()
    const { items } = useCartStore()

    const cartItems = Object.values(items)
    const totalItems = cartItems.reduce((acc, item) => acc + item.quantity, 0)
    const totalCost = cartItems.reduce((acc, item) => acc + item.product.price * item.quantity, 0)

    const handleSearch = (e) => setInputValue(e.target.value)

    useEffect(() => {
        if (inputValue === searchTerm) return

        const timeoutId = setTimeout(() => {
            setSearchTerm(inputValue)
            if (inputValue.trim() && location.pathname !== "/products") navigate("/products")
        }, 500)

        return () => clearTimeout(timeoutId)
    }, [inputValue, searchTerm, location.pathname, navigate, setSearchTerm])

    // useEffect(() => {
    //     setInputValue(searchTerm)
    // }, [searchTerm])

    return (
        <>
            <header className="sticky top-0 z-10 grid grid-cols-[1fr_auto] lg:grid-cols-[auto_auto_1fr_auto_auto] gap-2 lg:gap-12 items-center lg:border-b border-gray-200 max-lg:py-3 px-4 lg:pl-0 lg:pr-9 bg-white">
            <Link className="max-lg:hidden border-r border-gray-200 py-7 px-5" to="/">
                <svg width="134" height="30" viewBox="0 0 114 30" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14.3342 7.186C16.2619 7.186 17.9832 7.66644 19.4978 8.62732C21.0262 9.57447 22.2242 10.9197 23.0917 12.663C23.9316 14.3377 24.3516 16.3075 24.3516 18.5724C24.3516 20.7687 23.9316 22.7316 23.0917 24.4612C22.2517 26.1908 21.0675 27.5429 19.5391 28.5175C17.9969 29.5058 16.2619 30 14.3342 30C12.9297 30 11.6078 29.7117 10.3685 29.1352C9.12927 28.5587 8.06901 27.7488 7.18775 26.7056V29.4852H0V0H7.18775V10.4598C8.06901 9.41661 9.12927 8.61359 10.3685 8.05079C11.6078 7.47426 12.9297 7.186 14.3342 7.186ZM12.1861 24.0494C13.2051 24.0494 14.1139 23.8161 14.9125 23.3493C15.7112 22.8826 16.3377 22.2306 16.7921 21.3933C17.2465 20.5697 17.4737 19.6294 17.4737 18.5724C17.4737 17.5429 17.2465 16.6095 16.7921 15.7721C16.3377 14.9348 15.7112 14.2828 14.9125 13.8161C14.1139 13.3493 13.2051 13.116 12.1861 13.116C11.2223 13.116 10.3617 13.3493 9.60432 13.8161C8.84699 14.269 8.2549 14.9073 7.82804 15.731C7.40118 16.5683 7.18775 17.5154 7.18775 18.5724C7.18775 19.6294 7.40118 20.5765 7.82804 21.4139C8.2549 22.2375 8.84699 22.8826 9.60432 23.3493C10.3617 23.8161 11.2223 24.0494 12.1861 24.0494Z" fill="#F8CB46"></path><path d="M25.3356 29.4852V0H32.5233V29.4852H25.3356Z" fill="#F8CB46"></path><path d="M34.5607 29.4852V7.68016H41.7071V29.4852H34.5607Z" fill="#F8CB46"></path><path d="M57.2319 7.186C58.7603 7.186 60.1372 7.5429 61.3627 8.25669C62.5882 8.95676 63.5521 9.94509 64.2544 11.2217C64.9291 12.512 65.2664 13.9739 65.2664 15.6074V29.4852H58.4092V17.2135C58.4092 16.4173 58.2508 15.7104 57.9341 15.0927C57.6312 14.4612 57.1974 13.9739 56.6329 13.6307C56.0821 13.2876 55.4349 13.116 54.6914 13.116C53.9891 13.116 53.3419 13.2876 52.7498 13.6307C52.1577 13.9602 51.6965 14.4132 51.366 14.9897C51.0218 15.5388 50.8496 16.1839 50.8496 16.9252L50.8083 29.4852H43.6619V7.68016H50.8083V10.1716C51.483 9.23816 52.3849 8.51064 53.5141 7.98902C54.6432 7.45367 55.8824 7.186 57.2319 7.186Z" fill="#F8CB46"></path><path d="M81.0597 17.2135L89.1769 29.4852H81.0597L76.3091 21.7639L74.1198 24.2965V29.4852H66.932V0H74.1198V16.2869L81.0184 7.68016H89.1356L81.0597 17.2135Z" fill="#F8CB46"></path><path d="M34.5569 0.00232667H41.7267V5.59207H34.5569V0.00232667Z" fill="#F8CB46"></path><path d="M90.3176 29.4198V7.61479H97.464V29.4198H90.3176Z" fill="#54B226"></path><path d="M112.575 23.2634L114 27.855C113.353 28.4727 112.534 28.9737 111.542 29.3581C110.564 29.7424 109.607 29.9346 108.671 29.9346C107.322 29.9346 106.117 29.6395 105.057 29.0492C103.996 28.4452 103.17 27.6079 102.578 26.5372C101.986 25.494 101.69 24.2929 101.69 22.9339V13.3183H98.819V7.61479H101.69V0.00241089H108.547V7.61479H113.071V13.3183H108.547V21.6161C108.547 22.3162 108.733 22.8859 109.105 23.3251C109.477 23.7644 109.952 23.984 110.53 23.984C110.943 23.984 111.329 23.9223 111.687 23.7987C112.045 23.6752 112.341 23.4967 112.575 23.2634Z" fill="#54B226"></path><path d="M90.2609 0.00241089H97.4307V5.59215H90.2609V0.00241089Z" fill="#54B226"></path></svg>
            </Link>
            <div>
                <p className="text-lg font-extrabold">Delivery in 13 minutes</p>
                <p>Piura, Piura, Peru</p>
            </div>
            <div className="max-lg:col-span-full flex items-center rounded-xl border border-black/5 bg-gray-50 focus-within:shadow-xl">
                {isProductsRoute ? (
                    <button
                        type="button"
                        className="pl-2 cursor-pointer"
                        onClick={() => navigate("/")}
                        title="Go to home"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#1f1f1f"><path d="m313-440 224 224-57 56-320-320 320-320 57 56-224 224h487v80H313Z"/></svg>
                    </button>
                ) : (
                    <label className="pl-2" htmlFor="search">
                        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#1f1f1f"><path d="M784-120 532-372q-30 24-69 38t-83 14q-109 0-184.5-75.5T120-580q0-109 75.5-184.5T380-840q109 0 184.5 75.5T640-580q0 44-14 83t-38 69l252 252-56 56ZM380-400q75 0 127.5-52.5T560-580q0-75-52.5-127.5T380-760q-75 0-127.5 52.5T200-580q0 75 52.5 127.5T380-400Z"/></svg>
                    </label>
                )}
                <input 
                    onChange={handleSearch} 
                    className="flex-1 p-3 focus:outline-none focus:ring-0" 
                    type="search" 
                    name="search" 
                    id="search" 
                    placeholder='Search "sneaker"' 
                    value={inputValue} 
                />
            </div>
            {user ? (
                <div className="max-lg:col-start-2 max-lg:row-start-1 flex items-center gap-2">
                    <div className="flex flex-col items-end">
                        <span
                            className="max-w-24 sm:max-w-none truncate text-sm font-semibold text-zinc-700"
                            title={user.email}
                        >
                            {user.email}
                        </span>
                        <button 
                            onClick={logout}
                            className="text-xs text-blue-600 hover:underline"
                        >
                            Log out
                        </button>
                    </div>
                    <div className="w-8 h-8 rounded-full bg-green-600 flex items-center justify-center">
                        <svg className="w-5 h-5" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#ffffff"><path d="M234-276q51-39 114-61.5T480-360q69 0 132 22.5T726-276q35-41 54.5-93T800-480q0-133-93.5-226.5T480-800q-133 0-226.5 93.5T160-480q0 59 19.5 111t54.5 93Zm146.5-204.5Q340-521 340-580t40.5-99.5Q421-720 480-720t99.5 40.5Q620-639 620-580t-40.5 99.5Q539-440 480-440t-99.5-40.5ZM480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Z"/></svg>
                    </div>
                </div>
            ) : (
                <Link className="max-lg:col-start-2 max-lg:row-start-1 cursor-pointer" to="/login" state={{ backgroundLocation: location }}>
                    <svg className="lg:hidden w-8 h-8" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#1f1f1f"><path d="M234-276q51-39 114-61.5T480-360q69 0 132 22.5T726-276q35-41 54.5-93T800-480q0-133-93.5-226.5T480-800q-133 0-226.5 93.5T160-480q0 59 19.5 111t54.5 93Zm146.5-204.5Q340-521 340-580t40.5-99.5Q421-720 480-720t99.5 40.5Q620-639 620-580t-40.5 99.5Q539-440 480-440t-99.5-40.5ZM480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm100-95.5q47-15.5 86-44.5-39-29-86-44.5T480-280q-53 0-100 15.5T294-220q39 29 86 44.5T480-160q53 0 100-15.5ZM523-537q17-17 17-43t-17-43q-17-17-43-17t-43 17q-17 17-17 43t17 43q17 17 43 17t43-17Zm-43-43Zm0 360Z"/></svg>
                    <span className="max-lg:hidden text-xl text-zinc-700">Login</span>
                </Link>
            )}
            <button
                onClick={() => navigate("/cart", { state: { backgroundLocation: location } })}
                className={`max-lg:hidden flex items-center gap-2 rounded-lg p-3 font-bold text-white ${
                    totalItems > 0 ? "bg-green-700 hover:bg-green-800" : "bg-gray-200 hover:bg-gray-300"
                } transition`}
            >
                <svg className="w-6 h-6" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M223.5-103.5Q200-127 200-160t23.5-56.5Q247-240 280-240t56.5 23.5Q360-193 360-160t-23.5 56.5Q313-80 280-80t-56.5-23.5Zm400 0Q600-127 600-160t23.5-56.5Q647-240 680-240t56.5 23.5Q760-193 760-160t-23.5 56.5Q713-80 680-80t-56.5-23.5ZM246-720l96 200h280l110-200H246Zm-38-80h590q23 0 35 20.5t1 41.5L692-482q-11 20-29.5 31T622-440H324l-44 80h480v80H280q-45 0-68-39.5t-2-78.5l54-98-144-304H40v-80h130l38 80Zm134 280h280-280Z"/></svg>
                {totalItems > 0 ? (
                    <div className="flex flex-col text-start leading-tight">
                        <span>{totalItems} {totalItems === 1 ? "item" : "items"}</span>
                        <span className="text-xs font-semibold">${totalCost.toFixed(2)}</span>
                    </div>
                ) : (
                    <span>My Cart</span>
                )}
            </button>
        </header>
        {totalItems > 0 && (
            <button
                onClick={() => navigate("/cart", { state: { backgroundLocation: location } })}
                className="lg:hidden fixed bottom-3 left-3 right-3 p-4 flex items-center justify-between gap-4 z-20 rounded-2xl text-white bg-green-700 hover:bg-green-800 transition"
            >
                <div className="flex items-center gap-2">
                    <svg className="w-6 h-6" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M223.5-103.5Q200-127 200-160t23.5-56.5Q247-240 280-240t56.5 23.5Q360-193 360-160t-23.5 56.5Q313-80 280-80t-56.5-23.5Zm400 0Q600-127 600-160t23.5-56.5Q647-240 680-240t56.5 23.5Q760-193 760-160t-23.5 56.5Q713-80 680-80t-56.5-23.5ZM246-720l96 200h280l110-200H246Zm-38-80h590q23 0 35 20.5t1 41.5L692-482q-11 20-29.5 31T622-440H324l-44 80h480v80H280q-45 0-68-39.5t-2-78.5l54-98-144-304H40v-80h130l38 80Zm134 280h280-280Z"/></svg>
                    <div className="flex flex-col leading-tight">
                        <span>{totalItems} {totalItems === 1 ? "item" : "items"}</span>
                        <span className="text-sm">${totalCost.toFixed(2)}</span>
                    </div>
                </div>
                <span className="">
                    View Cart
                </span>
            </button>
        )}
        </>
    )
}

export default Header